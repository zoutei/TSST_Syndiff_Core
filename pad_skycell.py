...
class pad_skycell():
    ...
    def _load_ps1(self):
        if type(self.ps1) == str:
            self.ps1 = ps1_data(self.ps1,mask=False,toflux=True)
    ...
    def _pad_sides(self):
        ...
        for i in range(len(side_cells[0])):
            ...
            hdul = fits.open(os.path.join(self.datapath, projection, cell, filename))
            if len(hdul) == 1:
                j = 0 
            else:
                j = 1
            ...
            a = 2.5 / np.log(10)
            x = data / a
            flux = hdul[j].header["BOFFSET"] + hdul[j].header["BSOFTEN"] * 2 * np.sinh(x)
            buff = flux / hdul[j].header["EXPTIME"]
